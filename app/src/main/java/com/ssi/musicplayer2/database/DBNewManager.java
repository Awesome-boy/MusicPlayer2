package com.ssi.musicplayer2.database;
...
import static com.ssi.musicplayer2.database.DatabaseHelper.ID_COLUMN;
import static com.ssi.musicplayer2.database.DatabaseHelper.MUSIC_ID_COLUMN;


public class DBNewManager {

    private static final String TAG = DBNewManager.class.getName();
    private DatabaseHelper helper;
    private SQLiteDatabase db;
    private static DBNewManager instance = null;


    /* 因为getWritableDatabase内部调用了mContext.openOrCreateDatabase(mName, 0,mFactory);
     * 需要一个context参数 ,所以要确保context已初始化,我们可以把实例化DBManager的步骤放在Activity的onCreate里
     */
    public DBNewManager(Context context) {
        helper = new DatabaseHelper(context);
        db = helper.getWritableDatabase();
    }

    public static synchronized DBNewManager getInstance(Context context) {
        if (instance == null) {
            instance = new DBNewManager(context);
        }
        return instance;
    }
...
//                    musicInfo.setLove(love);
            ...
            e.printStackTrace();
            Log.d("zt","--Exception--"+e.getMessage());
        }
        return list;
    }
